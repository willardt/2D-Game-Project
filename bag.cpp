...
void Bag::createTexts() {
	...
	Text::printT(NULL, file.getU16(4), { 500, 75, 90, 25 }, _texts, Text::LEVEL, true);		//Level
	Text::printT(TEXT_DAMAGE, std::to_string(player->level), { 700, 75, NULL, 25 }, _texts, Text::LEVEL);
	Text::printT(NULL, file.getU16(5), { 500, 100, 50, 25 }, _texts, Text::EXP, true);		//EXP
	Text::printT(TEXT_DAMAGE, std::to_string(player->exp), { 700, 100, NULL, 25 }, _texts, Text::EXP);
	Text::printT(NULL, file.getU16(6), { 500, 125, 90, 25 }, _texts, Text::HEALTH, true);		//HEALTH
	Text::printT(TEXT_DAMAGE, std::to_string(player->health) + "/" + std::to_string(player->maxHealth), { 700, 125, NULL, 25 }, _texts, Text::HEALTH);
	Text::printT(NULL, file.getU16(7), { 500, 150, 50, 25 }, _texts, Text::HEALTH_REGEN, true);		//HPS
	Text::printT(TEXT_DAMAGE, std::to_string(player->hps), { 700, 150, NULL, 25 }, _texts, Text::HEALTH_REGEN);
	Text::printT(NULL, file.getU16(8), { 500, 175, 70, 25 }, _texts, Text::MANA, true);		//MANA
	Text::printT(TEXT_DAMAGE, std::to_string(player->mana) + "/" + std::to_string(player->maxMana), { 700, 175, NULL, 25 }, _texts, Text::MANA);
	Text::printT(NULL, file.getU16(9), { 500, 200, 50, 25 }, _texts, Text::MANA_REGEN, true);		//MPS
	Text::printT(TEXT_DAMAGE, std::to_string(player->mps), { 700, 200, NULL, 25 }, _texts, Text::MANA_REGEN);
	Text::printT(NULL, file.getU16(10), { 500, 225, 90, 25 }, _texts, Text::DAMAGE, true);		//DAMAGE
	Text::printT(TEXT_DAMAGE, std::to_string(player->damage), { 700, 225, NULL, 25 }, _texts, Text::DAMAGE);
	Text::printT(NULL, file.getU16(11), { 500, 250, 100, 25 }, _texts, Text::DEFENSE, true);		//DEFENSE
	Text::printT(TEXT_DAMAGE, std::to_string(player->defense), { 700, 250, NULL, 25 }, _texts, Text::DEFENSE);
	Text::printT(NULL, file.getU16(12), { 500, 275, 80, 25 }, _texts, Text::LEECH, true);		//LEECH
	Text::printT(TEXT_DAMAGE, std::to_string(player->leech), { 700, 275, NULL, 25 }, _texts, Text::LEECH);
	Text::printT(NULL, file.getU16(13), { 500, 300, 80, 25 }, _texts, Text::DRAIN, true);		//DRAIN
	Text::printT(TEXT_DAMAGE, std::to_string(player->drain), { 700, 300, NULL, 25 }, _texts, Text::DRAIN);
	Text::printT(NULL, file.getU16(14), { 500, 325, 70, 25 }, _texts, Text::LUCK, true);		//LUCK
	Text::printT(TEXT_DAMAGE, std::to_string(player->luck), { 700, 325, NULL, 25 }, _texts, Text::LUCK);
	Text::printT(NULL, file.getU16(15), { 500, 350, 80, 25 }, _texts, Text::SPEED, true);		//SPEED
	Text::printT(TEXT_DAMAGE, std::to_string(player->speed), { 700, 350, NULL, 25 }, _texts, Text::SPEED);
	Text::printT(NULL, file.getU16(16), { 500, 375, 90, 25 }, _texts, Text::SHARDS, true);		//SHARDS
	Text::printT(TEXT_DAMAGE, std::to_string(player->shards), { 700, 375, NULL, 25 }, _texts, Text::SHARDS);
}
...
void Bag::createItemTexts(const int& index) {
	...
	if (player->items[index].damage != 0) {
		...
		Text::printT(NULL, file.getU16(10), { 1025, height, NULL, 25 }, _itemText, Text::DAMAGE, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].damage), { 1250, height, NULL, 25 }, _itemText, Text::DAMAGE);
		height += 25;
	}

	if (player->items[index].defense != 0) {
		modifer = setModifer(player->items[index].defense);
		Text::printT(NULL, file.getU16(11), { 1025, height, 100, 25 }, _itemText, Text::DEFENSE, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].defense), { 1250, height, NULL, 25 }, _itemText, Text::DEFENSE);
		height += 25;
	}

	if (player->items[index].health != 0) {
		modifer = setModifer(player->items[index].health);
		Text::printT(NULL, file.getU16(6), { 1025, height, 100, 25 }, _itemText, Text::HEALTH, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].health), { 1250, height, NULL, 25 }, _itemText, Text::HEALTH);
		height += 25;
	}

	if (player->items[index].hps != 0) {
		modifer = setModifer(player->items[index].hps);
		Text::printT(NULL, file.getU16(7), { 1025, height, 100, 25 }, _itemText, Text::HEALTH_REGEN, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].hps), { 1250, height, NULL, 25 }, _itemText, Text::HEALTH_REGEN);
		height += 25;
	}

	if (player->items[index].mana != 0) {
		modifer = setModifer(player->items[index].mana);
		Text::printT(NULL, file.getU16(8), { 1025, height, 100, 25 }, _itemText, Text::MANA, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].mana), { 1250, height, NULL, 25 }, _itemText, Text::MANA);
		height += 25;
	}

	if (player->items[index].mps != 0) {
		modifer = setModifer(player->items[index].mps);
		Text::printT(NULL, file.getU16(9), { 1025, height, 100, 25 }, _itemText, Text::MANA_REGEN, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].mps), { 1250, height, NULL, 25 }, _itemText, Text::MANA_REGEN);
		height += 25;
	}

	if (player->items[index].leech != 0) {
		modifer = setModifer(player->items[index].leech);
		Text::printT(NULL, file.getU16(12), { 1025, height, 100, 25 }, _itemText, Text::LEECH, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].leech), { 1250, height, NULL, 25 }, _itemText, Text::LEECH);
		height += 25;
	}

	if (player->items[index].drain != 0) {
		modifer = setModifer(player->items[index].drain);
		Text::printT(NULL, file.getU16(13), { 1025, height, 100, 25 }, _itemText, Text::DRAIN, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].drain), { 1250, height, NULL, 25 }, _itemText, Text::DRAIN);
		height += 25;
	}

	if (player->items[index].luck != 0) {
		modifer = setModifer(player->items[index].luck);
		Text::printT(NULL, file.getU16(14), { 1025, height, 100, 25 }, _itemText, Text::LUCK, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].luck), { 1250, height, NULL, 25 }, _itemText, Text::LUCK);
		height += 25;
	}

	if (player->items[index].speed != 0) {
		modifer = setModifer(player->items[index].speed);
		Text::printT(NULL, file.getU16(15), { 1025, height, 100, 25 }, _itemText, Text::SPEED, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->items[index].speed), { 1250, height, NULL, 25 }, _itemText, Text::SPEED);
		height += 25;
	}

	if (player->items[index].duration != 0) {
		modifer = setModifer(player->items[index].duration);
		Text::printT(NULL, file.getU16(17), { 1025, height, 100, 25 }, _itemText, Text::DURATION, true);
		int duration = player->items[index].duration / 60;
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(duration), { 1250, height, NULL, 25 }, _itemText, Text::DURATION);
		height += 25;
	}
}
...
void Bag::createItemTextsEquip(const int& slot) {
	...
	if (player->equipped[slot].damage != 0) {
		...
		Text::printT(NULL, file.getU16(10), { 1025, height, 100, 25 }, _itemText, Text::DAMAGE, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].damage), { 1250, height, NULL, 25 }, _itemText, Text::DAMAGE);
		height += 25;
	}
	...
	if (player->equipped[slot].defense != 0) {
		...
		Text::printT(NULL, file.getU16(11), { 1025, height, 100, 25 }, _itemText, Text::DEFENSE, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].defense), { 1250, height, NULL, 25 }, _itemText, Text::DEFENSE);
		height += 25;
	}
	...
	if (player->equipped[slot].health != 0) {
		...
		Text::printT(NULL, file.getU16(6), { 1025, height, 100, 25 }, _itemText, Text::HEALTH, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].health), { 1250, height, NULL, 25 }, _itemText, Text::HEALTH);
		height += 25;
	}
	...
	if (player->equipped[slot].hps != 0) {
		...
		Text::printT(NULL, file.getU16(7), { 1025, height, 100, 25 }, _itemText, Text::HEALTH_REGEN, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].hps), { 1250, height, NULL, 25 }, _itemText, Text::HEALTH_REGEN);
		height += 25;
	}
	...
	if (player->equipped[slot].mana != 0) {
		...
		Text::printT(NULL, file.getU16(8), { 1025, height, 100, 25 }, _itemText, Text::MANA, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].mana), { 1250, height, NULL, 25 }, _itemText, Text::MANA);
		height += 25;
	}
	...
	if (player->equipped[slot].mps != 0) {
		...
		Text::printT(NULL, file.getU16(9), { 1025, height, 100, 25 }, _itemText, Text::MANA_REGEN, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].mps), { 1250, height, NULL, 25 }, _itemText, Text::MANA_REGEN);
		height += 25;
	}
	...
	if (player->equipped[slot].leech != 0) {
		...
		Text::printT(NULL, file.getU16(12), { 1025, height, 100, 25 }, _itemText, Text::LEECH, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].leech), { 1250, height, NULL, 25 }, _itemText, Text::LEECH);
		height += 25;
	}
	...
	if (player->equipped[slot].drain != 0) {
		...
		Text::printT(NULL, file.getU16(13), { 1025, height, 100, 25 }, _itemText, Text::DRAIN, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].drain), { 1250, height, NULL, 25 }, _itemText, Text::DRAIN);
		height += 25;
	}
	...
	if (player->equipped[slot].luck != 0) {
		...
		Text::printT(NULL, file.getU16(14), { 1025, height, 100, 25 }, _itemText, Text::LUCK, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].luck), { 1250, height, NULL, 25 }, _itemText, Text::LUCK);
		height += 25;
	}
	...
	if (player->equipped[slot].speed != 0) {
		...
		Text::printT(NULL, file.getU16(15), { 1025, height, 100, 25 }, _itemText, Text::SPEED, true);
		Text::printT(TEXT_DAMAGE, modifer + std::to_string(player->equipped[slot].speed), { 1250, height, NULL, 25 }, _itemText, Text::SPEED);
		height += 25;
	}
